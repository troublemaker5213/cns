...
class Block(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6):
        super().__init__()
        self.dwconv = nn.Conv3d(dim, dim, kernel_size=3, padding=3, groups=dim) # depthwise conv
...
if __name__ == '__main__':
    data = torch.randn((1,3,9,240,432))
    model = ConvNeXt(in_chans=3, depths=[1, 1, 3, 1], dims=[16, 32, 64, 128])
    with torch.no_grad():
        out = model(data)
    print(out)
